...
function savePNG() {
    let scatterplot_svgs = d3.select("#renderDiv").selectAll("svg");
    scatterplot_svgs.each(function () {
        save1Png(d3.select(this));
    });
    function save1Png(scatterplot_svg) {
...
function getColorName(x) {
    let c = getColorNameIndex(d3.rgb(x)),
        t = c3.color.relatedTerms(c, 1);
    if (t[0] === undefined) return "undefined";
    return c3.terms[t[0].index]
}

function getLuminance(x) {
    let c = rgb2hcl(x)
    return c.l;
}

function getSaturation(x) {
    // let c = d3.hsl(x)
    // return c.s
    let c = rgb2hcl(x)
    return c.c;
